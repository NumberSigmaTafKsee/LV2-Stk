...
class SW_ADSR : public GeneratorObject {
public:
	
    SW_ADSR(double sample_rate=44100.0f) : GeneratorObject()
    {        
        sr = sample_rate;
        reset();
        setAttackRate(0);
    ...
    SW_ADSR(double a, double d, double s, double r, double sample_rate=44100.0f, double cutoff=10.0f) {     
        sr = sample_rate;
        reset();
...
inline void SW_ADSR::gate(int gate) {
	if (gate)
		state = env_attack;
...
inline int SW_ADSR::getState() {
    return state;
}

inline void SW_ADSR::reset() {
    state = env_idle;
    output = 0.0;
}

inline double SW_ADSR::getOutput() {
	return output;
}

inline void SW_ADSR::setAttackRate(double rate) {
    attackRate = rate;
    attackCoef = calcCoef(rate, targetRatioA);
    attackBase = (1.0 + targetRatioA) * (1.0 - attackCoef);
}

inline void SW_ADSR::setDecayRate(double rate) {
    decayRate = rate;
    decayCoef = calcCoef(rate, targetRatioDR);
    decayBase = (sustainLevel - targetRatioDR) * (1.0 - decayCoef);
}

inline void SW_ADSR::setReleaseRate(double rate) {
    releaseRate = rate;
    releaseCoef = calcCoef(rate, targetRatioDR);
    releaseBase = -targetRatioDR * (1.0 - releaseCoef);
}

inline double SW_ADSR::calcCoef(double rate, double targetRatio) {
    return (rate <= 0) ? 0.0 : exp(-log((1.0 + targetRatio) / targetRatio) / rate);
}

inline void SW_ADSR::setSustainLevel(double level) {
    sustainLevel = level;
    decayBase = (sustainLevel - targetRatioDR) * (1.0 - decayCoef);
}

inline void SW_ADSR::setTargetRatioA(double targetRatio) {
...
inline void SW_ADSR::setTargetRatioDR(double targetRatio) {
    if (targetRatio < 0.000000001)
        targetRatio = 0.000000001;  // -180 dB
