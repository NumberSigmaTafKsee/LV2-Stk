...
private:
    ...
    float fFilterQ[3];
    DSP::VAStateVariableFilter fFilters[3];
...
void Rezonateur::init(double samplerate)
{
    ...
    for (unsigned i = 0; i < 3; ++i) {
        DSP::VAStateVariableFilter &filter = fFilters[i];
...
void Rezonateur::setOversampling(unsigned oversampling)
{
    ...
    for (unsigned b = 0; b < 3; ++b) {
        DSP::VAStateVariableFilter &filter = fFilters[b];
...
int Rezonateur::getFilterTypeForMode(int mode)
{
    ...
    default:
        ...
        /* fall through */
    case LowpassMode:
        return DSP::SVFLowpass;
    case BandpassMode:
    case BandpassNotchMode:
        return DSP::SVFBandpass;
    case HighpassMode:
        return DSP::SVFHighpass;
    }
}
