...
typedef struct _sk_bigverb_delay {
SKFLT *buf;
size_t sz;
int wpos;
int irpos;
...
int sp_bigverb_init(uint32_t sr, sp_bigverb *p)
{
    p->feedback = 0.97;
    p->lpfreq = 10000;
    p->sr = sr;
    p->bv = sk_bigverb_new(p->sr);    
    sk_bigverb_size(p->bv, p->feedback);
    sk_bigverb_cutoff(p->bv, p->lpfreq);
    return SP_OK;
}

int sp_bigverb_compute(sp_bigverb *p,
                       SPFLOAT *in1,
                       SPFLOAT *in2,
                       SPFLOAT *out1,
                       SPFLOAT *out2)
...
BigVerb* BigVerb_New(uint32_t sr)
{
  BigVerb* p = (BigVerb*)calloc(1,sizeof(BigVerb));
  assert(p != NULL);
  sp_bigverb_create(&p->bigverb);
  assert(p->bigverb != NULL);
  sp_bigverb_init(sr, p->bigverb);
  return p;
}
void BigVerb_Tick(BigVerb *p, float in1, float in2)
{
    p->bigverb->out1 = 0.0;
    p->bigverb->out2 = 0.0;
    sp_bigverb_compute(p->bigverb,&in1,&in2,&p->bigverb->out1,&p->bigverb->out2);    
}
%}