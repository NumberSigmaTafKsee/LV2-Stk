...
void buildUserInterfaceautowah(autowah* dsp, UIGlue* interface) {
	interface->addVerticalSlider(interface->uiInterface, "level", &dsp->fVslider2, 0.1f, 0.f, 1.f, 0.5f);
	interface->addVerticalSlider(interface->uiInterface, "wah", &dsp->fVslider0, 0.f, 0.f, 1.f, 0.5f);
	interface->addVerticalSlider(interface->uiInterface, "wet_dry", &dsp->fVslider1, 100.f, 0.f, 100.f, 50.f);
}
...
int sp_autowah_init(uint32_t sr, sp_autowah *p)
{
...
int sp_autowah_compute(sp_autowah *p, SPFLOAT *in, SPFLOAT *out)
{
    autowah *dsp = p->faust;
...
Autowah* Autowah_New(uint32_t sr)
{
    Autowah* a = (Autowah*)calloc(1,sizeof(Autowah));
    assert( a != NULL);
    sp_autowah_create(&a->awah);
    assert( a->awah != NULL);
    sp_autowah_init(sr,a->awah);   
    *a->awah->level = 0.9;
    *a->awah->mix   = 90.1;
    *a->awah->wah   = 0.5;
    return a;
...
float Autowah_Tick(Autowah * a, float in)
{
    float out = 0.0;
    sp_autowah_compute(a->awah, &in, &out);
    return out;
}
%}