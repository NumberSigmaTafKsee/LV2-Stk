...
Chorus::Chorus()
{
	...
	parameter_ = new double[kNumParams];
...
void Chorus::setSampleRate(double sampleRate)
{
	sampleRate_ = sampleRate;
}

void Chorus::setParameter(int index, double value)
{
	switch(index)
	{
	case kRate :
...
void Chorus::process(double &inL, double &inR)
{
		...
		// develop output wet
		inL = (double)(paramWetLevel * outL + (1 - paramWetLevel) * inL);
		inR = (double)(paramWetLevel * outR + (1 - paramWetLevel) * inR);
...
void Chorus::process(double &in)
{
		...
		// develop output wet
		in = (double)(paramWetLevel * outL + (1 - paramWetLevel) * in);
