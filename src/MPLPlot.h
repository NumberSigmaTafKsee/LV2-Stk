...
class Dictionary {
 public:
  ...
  void add(std::string key, double value) { double_values.insert(std::pair<std::string, double>(key, value)); };
...
class PlotCallOne {
 ...
 public:
  ...
  /// Save internal variables for this call to plot function, including the keyword argument function as a Dictionary
  /// instance
  ...
  /// return the string for this call, including the preceding import matplotlib.pyplot as plt and the following
  /// plt.show()
  std::string tostring() {
    std::string s;
    s += "y = [" + vector2string(y) + "]\n";
...
class PyPlotter {
 ...
 public:
  ...
  std::string print_calls() {
    std::string s;
    for (std::vector<PlotCall>::iterator it = PlotCalls.begin(); it != PlotCalls.end(); it++) { s += (*it).tostring(); }
    return s;
  }
