...
namespace Analog::Filters::Moog::StilsonMoogFilter
{
    struct StilsonMoog : public FilterProcessor
    {
        ...
        inline double crossfade( double amount, double a, double b ) {
            return (1-amount)*a + amount*b;
        }

        double fc,fs,Q,p;
        double cutoff,resonance;
        double lowpass,highpass,bandpass,lastX;
        double state[4], output; //should be gl obal scope / preserved between calls
        double pre_gain,post_gain;

        StilsonMoog(double Fc, double R, double Fs) : FilterProcessor() {        
            fs = Fs;        
            cutoff = Fc;
            resonance = R;
            pre_gain = 2.0f;
            ...
            setCutoff(Fc);
            setResonance(R);
            memset(&state[0],0,4*sizeof(double));
