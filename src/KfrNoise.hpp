...
namespace KfrDSP1
{
  ...
  class PinkNoise : public GneratorProcessor
  {
  public:
    PinkNoise() : GeneratorProcessor() {
    ...
    // you can multiple noise with Input I and Gain A
    // X and Y modulate the state and offset
    double Tick(double I=1, double G=1, double X=1, double Y=1) {
      static const double RMI2 = 2.0 / double(RAND_MAX); // + 1.0; // change for range [0,1)
      static const double offset = A[0] + A[1] + A[2];

    // unrolled loop
      double temp = double( I*noise.rand() );
      state[0] = P[0] * (state[0] - temp) + temp;
      temp = double( I*noise.rand() );
      state[1] = P[1] * (state[1] - temp) + temp;
      temp = double( I*noise.rand() );
      state[2] = P[2] * (state[2] - temp) + temp;
      return G*( A[0]*X*state[0] + A[1]*X*state[1] + A[2]*X*state[2] )*RMI2 - Y*offset;
    }
  ...
  double GaussianWhiteNoise()
  {
      ...
      return (double) sqrt( -2.0f * log( R1 )) * cos( 2.0f * M_PI * R2 );
  }
  ...
  class PinkingFilter : public FilterProcessor
  {
    double b0, b1, b2, b3, b4, b5, b6;
  public:
    PinkingFilter() : FilterProcessor(),b0(0), b1(0), b2(0), b3(0), b4(0), b5(0), b6(0) {}
    ...
    double Tick(double I=1, double A=1, double X=1, double Y=1)
    {
      ...
      b4 = 0.55000 * b4 + s * 0.5329522;
      b5 = -0.7616 * b5 - s * 0.0168980;
      const double pink = ( X*(b0 + b1 + b2 + b3 + b4 + b5 + b6) + Y*(s * 0.5362)) * 0.11;
      b6 = s * 0.115926;
      return A*pink;
    }
  };

  class BrowningFilter : public FilterProcessor
  {
  double l;
  public:
    BrowningFilter() : FilterProcessor(), l(0) {}
    double process(const double s)
    {
    ...
    {
      double brown = (X*l + Y*(0.02f * I)) / 1.02f;
      l = brown;
      return A*(brown * 3.5f); // compensate for gain
    }
  };
